...
public static class SchemaHelpers
{
    ...
    public static ClassDefinition? ExtractClass(this OpenApiSchema schema, string key, string @namespace, List<Guid> parents)
    {
        ...
        var classDef = new ClassDefinition
        {
            ...
            NestedClasses = schema.Properties.Where(p => p.Value.IsObject()).Select(p => p.Value.Reference == null ? p.Value.ExtractClass(p.Key, @namespace, parentsToSent) : null).Where(c => c != null).ToList(),
        ...
        var nestedClassesInArray = schema.Properties
            .Where(r => r.Value.Type == "array" && r.Value.Items.IsObjectWithoutReference())
            .Select(r => r.Value.Items.Reference == null ? r.Value.Items.ExtractClass(r.Key, @namespace, parentsToSent) : null)
            .Where(c => c != null)
            .ToList();
        var nestedClassesInPropertyAsOneOf = schema.Properties
            .Where(r => r.Value.IsObjectWithoutReference() && r.Value.OneOf.Any())
            .SelectMany(r => r.Value.OneOf.Where(a => a.Reference == null).Select(o => o.ExtractClass(r.Key + o.Title, @namespace, parentsToSent)))
            .Where(c => c != null)
            .ToList();
        ...
        var nestedClassesInPropertyAsAllOf = schema.Properties
            .Where(r => r.Value.IsObjectWithoutReference() && r.Value.AllOf.Any())
            .SelectMany(r => r.Value.AllOf.Where(a => a.Reference == null).Select(o => o.ExtractClass(r.Key + o.Title, @namespace, parentsToSent)))
            .Where(c => c != null)
            .ToList();
