...
internal static class OpenApiV3Deserializer
{
	...
	private static readonly PatternFieldMap<OpenApiDocument> _openApiPatternFields = new PatternFieldMap<OpenApiDocument> { 
	{
		...
		delegate(OpenApiDocument o, string p, ParseNode n, OpenApiDocument _)
		{
			if (p.Equals("x-oai-$self", StringComparison.OrdinalIgnoreCase))
			{
				...
				if (scalarValue != null)
				{
					o.Self = new Uri(scalarValue, UriKind.Absolute);
				}
			}
			else
			{
	...
	{
		{
			(string s) => s.Equals("x-oai-dataValue", StringComparison.OrdinalIgnoreCase),
		...
		{
			(string s) => s.Equals("x-oai-serializedValue", StringComparison.OrdinalIgnoreCase),
			delegate(OpenApiExample o, string p, ParseNode n, OpenApiDocument _)
			{
	...
	{
		(string s) => s.StartsWith("x-", StringComparison.OrdinalIgnoreCase),
		delegate(OpenApiMediaType o, string p, ParseNode n, OpenApiDocument t)
		{
	...
	{
		{
			(string s) => s.Equals("x-oai-deviceAuthorization", StringComparison.OrdinalIgnoreCase),
			delegate(OpenApiOAuthFlows o, string p, ParseNode n, OpenApiDocument t)
			{
	...
	private static readonly FixedFieldMap<OpenApiSchema> _openApiSchemaFixedFields = new FixedFieldMap<OpenApiSchema>
	{
		...
		},
		{
			...
			delegate(OpenApiSchema o, ParseNode n, OpenApiDocument _)
			{
				JsonSchemaType? jsonSchemaType = n.GetScalarValue()?.ToJsonSchemaType();
				if (o.Type.HasValue)
				{
		...
		},
		{
			...
			delegate(OpenApiSchema o, ParseNode n, OpenApiDocument doc)
			{
				o.OneOf = n.CreateList(LoadSchema, doc);
			}
		},
		{
			"anyOf",
		...
		},
		{
			...
			delegate(OpenApiSchema o, ParseNode n, OpenApiDocument _)
			{
				if (bool.TryParse(n.GetScalarValue(), out var result) && result)
				{
					if (o.Type.HasValue)
					{
	...
	{
		(string s) => s.StartsWith("x-", StringComparison.OrdinalIgnoreCase),
		delegate(OpenApiTag o, string p, ParseNode n, OpenApiDocument doc)
		{
	...
	public static IOpenApiCallback LoadCallback(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		if (referencePointer != null)
		{
			...
			return new OpenApiCallbackReference(referenceIdAndExternalResource.Item1, hostDocument, referenceIdAndExternalResource.Item2);
		}
	...
	public static IOpenApiExample LoadExample(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		{
			(string, string) referenceIdAndExternalResource = GetReferenceIdAndExternalResource(referencePointer);
			return new OpenApiExampleReference(referenceIdAndExternalResource.Item1, hostDocument, referenceIdAndExternalResource.Item2);
		}
	...
	public static IOpenApiHeader LoadHeader(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		{
			(string, string) referenceIdAndExternalResource = GetReferenceIdAndExternalResource(referencePointer);
			return new OpenApiHeaderReference(referenceIdAndExternalResource.Item1, hostDocument, referenceIdAndExternalResource.Item2);
		}
	...
	public static IOpenApiLink LoadLink(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		{
			(string, string) referenceIdAndExternalResource = GetReferenceIdAndExternalResource(referencePointer);
			return new OpenApiLinkReference(referenceIdAndExternalResource.Item1, hostDocument, referenceIdAndExternalResource.Item2);
		}
	...
	public static OpenApiOAuthFlow LoadOAuthFlow(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		foreach (PropertyNode item in mapNode)
		{
			item.ParseField(openApiOAuthFlow, _oAuthFlowFixedFields, _oAuthFlowPatternFields, hostDocument);
		}
	...
	public static IOpenApiPathItem LoadPathItem(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		{
			(string, string) referenceIdAndExternalResource = GetReferenceIdAndExternalResource(referencePointer);
			return new OpenApiPathItemReference(referenceIdAndExternalResource.Item1, hostDocument, referenceIdAndExternalResource.Item2);
		}
	...
	public static IOpenApiRequestBody LoadRequestBody(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		{
			(string, string) referenceIdAndExternalResource = GetReferenceIdAndExternalResource(referencePointer);
			return new OpenApiRequestBodyReference(referenceIdAndExternalResource.Item1, hostDocument, referenceIdAndExternalResource.Item2);
		}
	...
	public static IOpenApiSecurityScheme LoadSecurityScheme(ParseNode node, OpenApiDocument hostDocument)
	{
		...
		{
			(string, string) referenceIdAndExternalResource = GetReferenceIdAndExternalResource(referencePointer);
			return new OpenApiSecuritySchemeReference(referenceIdAndExternalResource.Item1, hostDocument, referenceIdAndExternalResource.Item2);
		}
	...
	private static void ParseMap<T>(MapNode? mapNode, T domainObject, FixedFieldMap<T> fixedFieldMap, PatternFieldMap<T> patternFieldMap, OpenApiDocument hostDocument)
	{
		if (mapNode == null)
		{
			return;
		...
		foreach (PropertyNode item in mapNode)
		{
			item.ParseField(domainObject, fixedFieldMap, patternFieldMap, hostDocument);
		}
	}
	...
	private static IOpenApiExtension LoadExtension(string name, ParseNode node)
	{
		...
		{
			IOpenApiExtension openApiExtension = value(node.CreateAny(), OpenApiSpecVersion.OpenApi3_0);
			if (openApiExtension != null)
			{
